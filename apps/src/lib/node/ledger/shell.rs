...
impl Shell {
    ...
    pub fn init_chain(
        ...
        let fountain_vp = std::fs::read("wasm/vp_fountain.wasm")
            .expect("cannot load fountain VP");

        // TODO load initial accounts from genesis

        // temporary account addresses for testnet, generated by the
        // address.rs module
        ...
        for (user, token) in users.iter().cartesian_product(tokens.keys()) {
            ...
            self.storage
                .write(&Key::validity_predicate(user), fountain_vp.to_vec())
